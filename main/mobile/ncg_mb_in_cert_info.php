...
<!-- BEGIN LOADER -->
    ...
    <div class="helpdesk container" >
         ...
         <nav class="navbar navbar-expand navbar-light">
            <a class="navbar-brand" href="ncg_mb_in_project_info?pid=<?php echo $pid?>" style="width:70px; border-radius: 25px; background-color: #dddddd; padding-left: 4px; margin: 0px; height: 35px; line-height: 100%;"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item active">
                        <a class="nav-link ellipsis" href="ncg_mb_in_project_info?pid=<?php echo $pid?>"><?php echo NCG_FUNCT::GET_MB_BADGE()." ".$payment_certificate['CERT_NUM']?></a>
                    </li>
                </ul>
            </div>
        </nav>
                            ...
                            <div class="statbox widget box box-shadow">
                                <div class="widget-content widget-content-area animated-underline-content">
                                    ...
                                    <ul class="nav nav-tabs  mb-3 navbar navbar-expand navbar-light" id="animateLine" role="tablist" style="margin-top: 50px; z-index: 99999; background-color: #fff; box-shadow: 0px 11px 7px 3px rgb(0 0 0 / 0%), 0 0px 0px 0 rgba(0, 0, 0, 0.12), 0 6px 20px -5px rgba(0, 0, 0, 0.2);">
                                        <li class="nav-item">
                                            <a class="nav-link" id="animated-underline-pay-cert-tab" data-toggle="tab" href="#animated-underline-pay-cert" role="tab" aria-controls="animated-underline-pay-cert" aria-selected="false" onclick="switchRef('b')">Payment Certificate Info</a>
                                        </li>
                                    </ul>

                                    <div class="tab-content" id="animateLineContent-4" style="margin-top: 55px; margin-bottom: 100px;">
                                        <div class="tab-pane fade active show" id="animated-underline-pay-cert" role="tabpanel" aria-labelledby="animated-underline-pay-cert-tab">


                                            <p class="mb-4" style="background-color: #F0F0F0; border: 1px solid #000; padding: 10px; ">
                                                  <strong>Contract Value: </strong><?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($project_finances['CURRENT_VALUE'])?>
                                                  <br>
                                                  <br>
                                                  <strong>Total Claimed Amnt: </strong><?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($project_finances['CLAIMED'])?>
                                                  <br>
                                                  <strong>Total Paid Amnt: </strong><?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($project_finances['PAID'])?>
                                                  <br>
                                                  <strong>Total Owed Amnt: </strong><?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($project_finances['CLAIMED']-$project_finances['PAID'])?>
                                                  <br>
                                                  <br>

                                                  <strong>Works Remaining: </strong><?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($project_finances['CURRENT_VALUE'] - $project_finances['CLAIMED'])?>
                                                </p>

                                            <?php 
                                              $payments = NCG_FUNCT::GET_PAYMENTS($payment_certificate['CERT_NUM'],$pid);
                                              $totalPaid = 0;
                                              $remainig = $payment_certificate['CERT_AMOUNT'];

                                              if(mysqli_num_rows($payments)>0){
                                              while($payment = $payments ->fetch_assoc()){
                                                $user_info = NCG_FUNCT::GET_USER_INFO($payment['ADDED_BY']);

                                                  $payDate = date("d M Y ", strtotime($payment["PAY_DATE"]));
                                                  $totalPaid += $payment['AMOUNT'];
                                                  $remainig -= $payment['AMOUNT'];

                                              ?>
                                              <hr style=" background-color: #000!important" />
                                                <p>
                                                <span style="font-weight: lighter; color: #258AED; font-size: 18px;"> Payment <span style="color: #000;"><?php echo $payment['PAY_ID'] ?></span></span>
                                                <br>
                                                <br>
                                                <span style="font-weight: bolder; color: #258AED;"> Payment Date: <span style="color: #000;"> <?=$payDate?></span></span>
                                                <br>
                                                  <span style="font-weight: bolder; color: #258AED">Paid Amount: </span> <?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($payment['AMOUNT'])?>
                                                <br>
                                                <br>
                                                  <span style="font-weight: bolder; color: #258AED">Total Paid Amount: </span> <?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($totalPaid)?>
                                                <br>
                                                <br>
                                                  <span style="font-weight: bolder; color: #258AED">Owed Amount: </span> <?php echo $_GET['C']." ".NCG_FUNCT::MINIFY_NUMBER($remainig)?>
                                                <br>
                                                <br>
                                                  <span style="font-weight: bolder; color: #258AED">Descr: </span> <?php echo $payment['DESCRIPTION']?>
                                                  <br>
                                                </p> 
                                              ...
                                              <?php
                                                 ...
                                                 <h4 class="mb-4">
                                                  <center>No payments made.</center>
                                                </h4>
                                                <?php
                                              }
                                                ?>
                                         ...
                                         <div class="row">
                                          <div class="col-lg-6">
                                            ...
                                            <span class="input-group-text">Pay Cert Date</span>
                                                ...
                                                <input id="payment-id" class="form-control flatpickr flatpickr-input active" type="text" name="payDate" placeholder="Select Date.." required>
                                            </div>
                                          </div>
             ...
             <div class="container">
                 <div class="row">
                     <div class="navbar navbar-expand navbar-light" style="width: 100%;">
                       <a class="btn btn-primary" href="javascript:void(0);" data-toggle="modal" data-target="#new-payment" style="width: 100%; padding: 5px; text-align: center" id="click-btn">NEW Payment</a>
                   </div>
                     </div>      
                 </div>
           </div>
    ...
    <script type="text/javascript">
      ...
      var topup = flatpickr(document.getElementById('payment-id'));
